...
function getAbogusUrl(url, userAgent) {
    const msToken = generateMsToken(107);

    const query = (`${url}&msToken=${msToken}`).split("?")[1];
    ...
    vm.runInContext(kABogus, context, { timeout: 1000 });

    if (typeof context.getABogus !== "function") {
        throw new Error("getABogus not found");
    }
    ...
    return (
        `${url}&msToken=${encodeURIComponent(msToken)}` +
        `&a_bogus=${encodeURIComponent(String(aBogus))}`
    );
}
    ...
    req.on("end", () => {
        try {
            ...
            if (req.url === "/sign") {
                ...
                const roomKey = Object.keys(payload.data).find(k =>
                    k.startsWith("room")
                );
